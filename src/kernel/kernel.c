...
void render_desktop(mouse_state_t *mouse)
{
    ...
    // Warna hijau terang jika Start Menu sedang terbuka atau tombol ditekan
    ...
    // 4. Render Start Menu (di atas Window & Taskbar)
    menu_draw(&start_menu, mouse->x, mouse->y);

    // 5. Render Kursor Mouse paling depan
...
void kernel_main(unsigned int magic, multiboot_info_t *mb_info)
{
    ...
    while (1)
    {
        ...
        // 1. Logika Klik Mouse (Trigered saat tombol kiri baru ditekan)
        ...
        if (mouse->left_button && !prev_start_click)
        {
            ...
            {
                // Cek apakah klik berada di dalam Start Menu
                int action = menu_handle_click(&start_menu, mouse->x, mouse->y);

                if (action == 1)
                {                              // About OS
                    main_win.is_visible = 1;   // Buka window utama
                    start_menu.is_visible = 0; // Auto-close Start Menu
                }
                else if (action == 2)
                {                              // Terminal
                    start_menu.is_visible = 0; // Auto-close Start Menu
                ...
                {
                    // Klik dilakukan di luar Start Menu:
                    // 1. Tutup Start Menu
                    start_menu.is_visible = 0;
                    // 2. Teruskan klik ke Window agar tombol 'X' / Drag langsung merespons
                    window_handle_mouse(&main_win, mouse->x, mouse->y, mouse->left_button);
                }
            }
            else
            {
                // Handle Window Drag / Close normal saat Start Menu tidak terbuka
                window_handle_mouse(&main_win, mouse->x, mouse->y, mouse->left_button);
            }
        }
        else if (mouse->left_button && main_win.is_dragging)
        {
            // Terapkan posisi baru saat window sedang didrag
            window_handle_mouse(&main_win, mouse->x, mouse->y, mouse->left_button);
        }
        else if (!mouse->left_button && main_win.is_dragging)
        {
            // Lepas status dragging jika tombol mouse dilepas
            main_win.is_dragging = 0;
        }

        prev_start_click = mouse->left_button;

        // 2. Render ulang jika ada perubahan posisi/klik
